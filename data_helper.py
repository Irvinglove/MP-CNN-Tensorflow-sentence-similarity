...
def read_data_sets(train_dir,FLAGS):
    #
    # s1代表数据集的句子1
    # s2代表数据集的句子2
    # score代表相似度
    ...
    score = []
    s_count = 0
    for line in open(FLAGS.sick_dir, 'r'):
        arr = line.split("\t")
    ...
    for i in range(s_count):
        ...
        #
        # score_ith = int(float(score[i]))
        # score_vec = [0,0,0,0,0]
        # score_vec[score_ith - 1] = 1
        # label.append(np.array(score_vec))
        label.append(np.array(np.array([score[i]]).astype(float)))
    print "求得每个句子对应的索引序列,并返回"
...
if __name__ == "__main__":
    ...
    FLAGS.sick_dir = "SICK_data/SICK.txt"
    read_data_sets('0',FLAGS)